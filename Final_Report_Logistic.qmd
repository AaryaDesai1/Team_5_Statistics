...
```{r}
library(dplyr)
# Summary statistics for age, sex, chest pain type, cholesterol grouped by heart disease status
logistic_data %>%
  group_by(`Lifestyle Category`) %>%
  summarize(
    Mean_Year = mean(Year),  # Replace Age with actual variable name
    Median_Year = median(Year),  # Replace Age with actual variable name
    Mean_IC = mean(Income_Category),  # Replace Cholesterol with actual variable name
    Median_IC = median(Income_Category)  # Replace Cholesterol with actual variable name
    # Add more summary statistics for other variables as needed
  )
